...
namespace robogen {
  ...
  class BodyConnectionVisitor: public boost::default_bfs_visitor {
    ...
    public:
      ...
      BodyConnectionVisitor(dWorldID& odeWorld,
          dJointGroupID connectionJointGroup, Swarm *robot) :
        odeWorld_(odeWorld), 
  ...
  bool Swarm::init(dWorldID odeWorld, dSpaceID odeSpace,
  ...
  const std::vector<boost::shared_ptr<Sensor> >& Swarm::getSensors() const {
    return sensors_;
  }

  const std::vector<boost::shared_ptr<Motor> >& Swarm::getMotors() {
    return motors_;
  }

  const std::vector<boost::shared_ptr<Model> >& Swarm::getBodyParts() {
    return bodyParts_;
  }

  const boost::shared_ptr<NeuralNetwork>& Swarm::getBrain() const {
    return neuralNetwork_;
  } // Decode the body connections

  boost::shared_ptr<Model> Swarm::getCoreComponent() {
    return coreComponent_;
  }

  bool Swarm::decodeBody(const robogenMessage::Body& robotBody) {
  ...
  bool Swarm::decodeBrain(const robogenMessage::Brain& robotBrain) {
  ...
  int Swarm::getId() const {
    return id_;
  }

  const robogenMessage::Swarm& Swarm::getMessage() {
    return *robotMessage_;
  }

  const std::vector<boost::shared_ptr<Connection> >& Swarm::getBodyConnections() const {

    return bodyConnections_;

  }

  void Swarm::traverseBody(const std::vector<boost::shared_ptr<Model> >,
